class ShakespeareanInsults{
    ...
    createRandomInsult():string {
        let one: number = Math.round(Math.random()*(this.partOne.length-1));
        let two: number = Math.round(Math.random()*(this.partTwo.length-1));
        let three: number = Math.round(Math.random()*(this.partThree.length-1));
        return "Thou " + this.partOne[one] + " " + this.partTwo[two] + " " + this.partThree[three];
    }

    getClassicInsult():string{
        let random: number = Math.round(Math.random()*(this.classic.length-1));
        return this.classic[random];
    }

}

let insults: ShakespeareanInsults = new ShakespeareanInsults();
